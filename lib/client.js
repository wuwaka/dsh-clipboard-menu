...
function engineItems(anchor, reopen) {
  ...
  if (readCustomTemplate()) {
    // Only worth offering once there is something to delete.
    custom.action = {
      icon: ICONS.trash,
      title: L.remove,
      run: function () { deleteCustomEngine(reopen); }
    };
  }
  items.push(custom);
  return items;
...
/** Forget the stored provider; the engine falls back to the language guess. */
function deleteCustomEngine(reopen) {
  storeCustomTemplate("");
  storeCustomName("");
  if (readStoredEngine() === "custom") storeEngine("");
  if (reopen) reopen();
  else closeMenu();
}
