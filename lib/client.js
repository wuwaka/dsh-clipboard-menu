...
/** Custom providers live in one list; the first version stored only a single one. */
var CUSTOMS_KEY = "dsh-clipboard-menu.customs";
var LEGACY_CUSTOM_KEY = "dsh-clipboard-menu.custom";
var LEGACY_NAME_KEY = "dsh-clipboard-menu.customName";
...
/** The engine id that selects one stored provider. */
function customKey(id) {
  return "custom:" + id;
}
function readCustoms() {
  try {
    var list = JSON.parse(window.localStorage.getItem(CUSTOMS_KEY) || "[]");
    if (!Array.isArray(list)) return [];
    return list.filter(function (e) { return e && typeof e.url === "string" && e.url; });
  } catch (e) {
    return [];
  }
}
function writeCustoms(list) {
  try { window.localStorage.setItem(CUSTOMS_KEY, JSON.stringify(list)); } catch (e) {}
}
/** Adopt the single provider an earlier version stored, exactly once. */
function migrateLegacyCustom() {
  var legacy = "";
  var name = "";
  try {
    legacy = window.localStorage.getItem(LEGACY_CUSTOM_KEY) || "";
    name = window.localStorage.getItem(LEGACY_NAME_KEY) || "";
  } catch (e) {}
  if (!legacy || readCustoms().length) return;
  var id = newCustomId();
  writeCustoms([{ id: id, name: name, url: legacy }]);
  try {
    window.localStorage.removeItem(LEGACY_CUSTOM_KEY);
    window.localStorage.removeItem(LEGACY_NAME_KEY);
    if (readStoredEngine() === "custom") storeEngine(customKey(id));
  } catch (e) {}
}
...
function currentEngine() {
  ...
  if (stored.indexOf("custom:") === 0) {
    var wanted = stored.slice("custom:".length);
    var list = readCustoms();
    for (var i = 0; i < list.length; i++) {
      if (list[i].id === wanted) {
        return {
          id: stored,
          name: list[i].name || engineNameFromUrl(list[i].url) || labels().custom,
          url: list[i].url
        };
      }
    }
...
function engineItems(anchor, reopen) {
  ...
  items.push({ sep: true });
  readCustoms().forEach(function (entry) {
    var key = customKey(entry.id);
    items.push({
      label: entry.name || engineNameFromUrl(entry.url) || L.custom,
      icon: ICONS.search,
      hint: cur.id === key ? L.current : "",
      run: function () {
        storeEngine(key);
        closeSubmenu();
        if (reopen) reopen();
      ...
      action: {
        icon: ICONS.trash,
        title: L.remove,
        run: function () { deleteCustomEngine(entry.id, reopen); }
      }
    });
  });
  items.push({
...
/** Forget one stored provider; the engine falls back to the language guess. */
function deleteCustomEngine(id, reopen) {
  var key = customKey(id);
  writeCustoms(readCustoms().filter(function (e) { return e.id !== id; }));
  if (readStoredEngine() === key) storeEngine("");
  if (reopen) reopen();
  else closeMenu();
}
...
function showCustomEngineForm(anchor, reopen) {
  ...
  // Always an "add" form: the list already offers select and delete per provider.
  var nameInput = makeField("", L.custom);
  var urlInput = makeField("", "https://example.com/search?q={query}");
  ...
  function commit() {
    ...
    if (!template) return;
    var entry = { id: newCustomId(), name: String(nameInput.value || "").trim(), url: template };
    var list = readCustoms();
    list.push(entry);
    writeCustoms(list);
    storeEngine(customKey(entry.id));
    closeSubmenu();
    if (reopen) reopen();
  }

  // Paste lives in the fields' right-click menu, so there is no Paste button.
...
function ensureStyle() {
  ...
  st.textContent =
    ...
    "[" + MENU_ATTR + "] .dcm-row{display:flex;align-items:center}" +
    "[" + MENU_ATTR + "] .dcm-row .dcm-item{flex:1 1 auto;min-width:0}" +
    "[" + MENU_ATTR + "] .dcm-action{display:none;flex:0 0 auto;margin-right:5px;padding:4px;" +
    "border:0;border-radius:5px;background:transparent;color:inherit;cursor:default;opacity:.55;line-height:0}" +
    "[" + MENU_ATTR + "] .dcm-row:hover .dcm-action{display:inline-flex}" +
    "[" + MENU_ATTR + "] .dcm-action:hover{opacity:1;background:Highlight;color:HighlightText}" +
    "[" + MENU_ATTR + "] .dcm-action svg{display:block;width:14px;height:14px;fill:none;" +
...
/**
 * A left-click dismisses every layer above the one it lands in, and everything
 * when it lands outside them all. The layers nest: the field menu sits on the
 * submenu (the engine list or custom form), which sits on the main menu. The
 * earlier "return early on whichever layer contains the target" version left
 * upper layers stranded when the click landed lower down.
 */
function onDocMouseDown(ev) {
  var t = ev.target;
  if (fieldMenuEl && fieldMenuEl.contains(t)) return;
  if (fieldMenuEl) closeFieldMenu();
...
function buildMenu(items, dismiss) {
  ...
  items.forEach(function (it) {
    ...
    if (it.action) {
      // A trailing affordance that only shows while its row is hovered. It lives
      // beside the row button, never inside it, so both stay valid buttons.
      var wrap = document.createElement("div");
      wrap.className = "dcm-row";
      wrap.appendChild(b);
      var act = document.createElement("button");
      act.type = "button";
      act.className = "dcm-action";
      act.title = it.action.title || "";
      if (it.action.title) act.setAttribute("aria-label", it.action.title);
      act.innerHTML = it.action.icon;   // constant markup from ICONS
...
function showMenu(x, y, items) {
  ...
  // Registered synchronously: deferring this to a timer left a window where
  // clicks just after opening were ignored. No defer is needed — the menu opens
  // on contextmenu, whose own mousedown has already been dispatched.
