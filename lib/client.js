...
function currentEngine() {
  var custom = window.__DSH_CLIPBOARD_MENU_SEARCH__;
  if (typeof custom === "string" && custom) return { id: "custom", name: labels().custom, url: custom };
  return findEngine(readStoredEngine()) || findEngine(langCode().indexOf("zh") === 0 ? "baidu" : "google") || FALLBACK_ENGINE;
}
...
function runSearch(text) {
  ...
  if (!q) return false;
  var url = currentEngine().url + encodeURIComponent(q);
  try {
...
/**
 * Let the operator choose an engine. Picking one stores it and reopens the menu
 * it was opened from — rebuilt, so the engine hint reflects the new choice —
 * rather than dropping the operator out of the menu entirely.
 */
function showEngineChooser(x, y, reopen) {
  var L = labels();
  var cur = currentEngine();
  showMenu(x, y, ENGINES.map(function (en) {
    return {
      label: en.name,
      icon: ICONS.search,
...
function onContextMenu(ev) {
  ...
  if (!root) {
    ...
    var buildReadOnly = function () {
      return [
        ...
        { label: LO.engine, icon: ICONS.engine, run: function () { showEngineChooser(rx, ry, showReadOnly); } }
      ];
    };
  ...
  var buildComposer = function () {
    return [
      ...
      { label: L.engine, icon: ICONS.engine, run: function () { showEngineChooser(mx, my, showComposer); } }
    ];
  };
  // "reopen" has to re-render, not merely rebuild: pass the showing action.
