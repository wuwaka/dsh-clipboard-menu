...
function currentEngine() {
  ...
  if (stored === "custom") {
    ...
    if (template) return { id: "custom", name: engineNameFromUrl(template) || labels().custom, url: template };
  }
...
var EN = { cut: "Cut", copy: "Copy", paste: "Paste", selectAll: "Select All",
           ...
           customUrl: "Custom search engine URL",
...
/**
 * Inline field for a custom engine. Enter stores the template and reopens the
 * parent menu; Escape backs out one level. window.prompt is unavailable in
 * Electron, so the field lives in the submenu itself.
 */
function showCustomEngineForm(anchor, reopen) {
  ...
  el.style.minWidth = "268px";

  var lab = document.createElement("span");
  lab.className = "dcm-form-label";
  lab.textContent = L.customUrl;

  var inp = document.createElement("input");
  inp.type = "text";
  inp.className = "dcm-input";
  inp.spellcheck = false;
  inp.value = readCustomTemplate();
  inp.placeholder = "https://example.com/search?q={query}";
  ...
  el.style.left = Math.max(8, Math.min(r.right - box.width, window.innerWidth - box.width - 8)) + "px";
  el.style.top = Math.max(8, Math.min(r.bottom, window.innerHeight - box.height - 8)) + "px";
...
/** Build menu DOM from item specs; shared by the main menu and its submenus. */
...
/** Keep a floating panel inside the viewport, then attach it. */
...
/** A second layer hanging off the bottom-right of the row it belongs to. */
