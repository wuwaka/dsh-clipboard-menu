...
var menuPos = { x: 0, y: 0 };  // where the open menu sits, so a follow-up menu can reuse it
var toastEl = null;
var toastTimer = 0;
...
/** Let the operator choose an engine; the choice outlives the session. */
function showEngineChooser(x, y) {
  var L = labels();
  ...
  showMenu(x, y, ENGINES.map(function (en) {
    return {
      ...
      run: function () { storeEngine(en.id); }
    };
  }));
}
...
function onContextMenu(ev) {
  ...
  if (!root) {
    ...
    showMenu(ev.clientX, ev.clientY, [
      ...
      { label: LO.engine, icon: ICONS.engine, run: function () { showEngineChooser(menuPos.x, menuPos.y); } }
    ]);
    return;
  }
  ...
  var items = [
    ...
    { label: L.engine, icon: ICONS.engine, run: function () { showEngineChooser(menuPos.x, menuPos.y); } }
  ];
